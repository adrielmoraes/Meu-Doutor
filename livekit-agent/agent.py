        ...
        try:
            ...
            try:
                ...
                if len(rgba_data) != expected_size:
                    logger.error(
                        f"[Vision] RGBA data size mismatch: expected {expected_size}, got {len(rgba_data)}"
                    )
                ...
                logger.info(
                    f"[Vision] ✅ Converted to RGB via RGBA successfully")

            except Exception as convert_err:
                logger.error(
                    f"[Vision] Frame conversion failed: {convert_err}")
                import traceback
                traceback.print_exc()
