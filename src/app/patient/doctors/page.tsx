...
import { getDoctors, getPatientById, getAppointmentsForPatient } from "@/lib/db-adapter";
...
const DoctorCard = ({ doctor, patientId }: { doctor: Doctor, patientId: string }) => {
    const privacySettings = (doctor as any).settings?.privacy;
    // If showOnlineStatus is explicitly false, force offline appearance
    const isOnlineVisible = privacySettings?.showOnlineStatus === false ? false : doctor.online;
            ...
            <CardHeader className="flex-grow">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        {/* Privacy Setting Applied Here */}
                        <div className={`mt-1 text-xs font-semibold inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 ${isOnlineVisible ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-600'}`}>
                            <span className={`h-2 w-2 rounded-full ${isOnlineVisible ? 'bg-green-500' : 'bg-slate-400'}`}></span>
                            {isOnlineVisible ? 'Online' : 'Offline'}
                        </div>
                    </div>
                </div>
            </CardHeader>
