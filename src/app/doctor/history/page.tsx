    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-950 min-h-screen relative font-sans text-slate-900 dark:text-slate-50">
            ...
            <div className="relative z-10 container mx-auto p-4 md:p-8">
                ...
                {error ? (
                    <Alert variant="destructive" className="bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800 text-red-800 shadow-sm border-none ring-1 ring-red-200">
                        ...
                        <Table>
                            <TableHeader className="bg-slate-50 dark:bg-slate-950/50">
                                <TableRow className="hover:bg-transparent border-slate-100 dark:border-slate-800">
                                    <TableHead className="text-slate-900 dark:text-slate-50 font-extrabold h-14">Paciente</TableHead>
                                    <TableHead className="hidden md:table-cell text-slate-900 dark:text-slate-50 font-extrabold h-14">Data do Atendimento</TableHead>
                                    <TableHead className="text-slate-900 dark:text-slate-50 font-extrabold h-14">Diagnóstico Validado</TableHead>
                                    <TableHead className="text-right text-slate-900 dark:text-slate-50 font-extrabold h-14 pr-8">Ações</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {history.map(entry => (
                                    <TableRow key={entry.id} className="hover:bg-blue-50 dark:bg-blue-950/30/30 transition-colors border-slate-50">
                                        <TableCell className="py-5 pl-8">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <span className="font-bold text-slate-900 dark:text-slate-50">{entry.name}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell className="hidden md:table-cell py-5">
                                            <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 dark:text-slate-600 px-3 py-1 rounded-md text-sm font-bold">
                                                {entry.lastVisit}
                                            </span>
                                        </TableCell>
                                        <TableCell className="py-5">
                                            <p className="font-medium text-slate-700 dark:text-slate-200 leading-relaxed max-w-md line-clamp-2">
                                ...
                                {history.length === 0 && (
                                    <TableRow>
                                        <TableCell colSpan={4} className="text-center text-slate-500 dark:text-slate-400 py-16">
