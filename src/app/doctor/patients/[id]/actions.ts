...
import { getPatientById, updatePatient, updateExam, getExamsByPatientId, saveConsultation } from '@/lib/db-adapter';
...
export async function saveSoapEvolutionAction(patientId: string, soapData: { subjective: string; objective: string; assessment: string; plan: string }) {
    try {
...
P: ${soapData.plan}
        ...
        await saveConsultation(
            session.userId,
            patientId,
            '', // No room ID for manual evolution
            '', // No transcription
            summary,
            'evolution'
        );
        ...
        revalidatePath(`/doctor/patients/${patientId}`);
        return { success: true, message: 'Evolução salva com sucesso!' };
    } catch (error) {
        console.error('Failed to save SOAP evolution:', error);
        return { success: false, message: 'Erro ao salvar a evolução clínica.' };
    }
}
