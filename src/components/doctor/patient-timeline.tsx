...
export default function PatientTimeline({ exams, consultations, prescriptions, patient }: PatientTimelineProps) {
    ...
    return (
        <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 h-full shadow-sm border-none ring-1 ring-slate-200 dark:ring-slate-700">
                ...
                <div className="relative pl-4 border-l-2 border-slate-100 dark:border-slate-800 space-y-8 ml-2">
                    ...
                    {patient.lastVisit && (
                        <div className="relative">
                            ...
                            <div className="flex flex-col">
                                ...
                                <span className="text-sm text-slate-900 dark:text-slate-50 font-semibold">
                    ...
                    {timelineItems.map((item, index) => (
                        <div key={`${item.type}-${index}`} className="relative group">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 hover:border-blue-200 dark:border-blue-800 hover:shadow-md transition-all duration-300">
                                    {item.type === 'exam' && (
                                        <>
                                            <div className="flex items-center justify-between mb-2">
                                                <span className="text-sm font-bold text-slate-900 dark:text-slate-50 flex items-center gap-2">
                                                ...
                                                <Badge variant="outline" className={`text-[10px] h-5 px-2 font-semibold ${item.data.status === 'Validado'
                                                        ...
                                                        : 'bg-amber-50 dark:bg-amber-950/30 text-amber-700 border-amber-100'
                                                    }`}>
                                                    {item.data.status}
                                                </Badge>
                                            </div>
                                            <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 line-clamp-3 leading-relaxed">
                                    ...
                                    {item.type === 'consultation' && (
                                        <>
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <Badge variant="outline" className="text-[10px] h-5 px-2 font-semibold bg-blue-50 dark:bg-blue-950/30 text-blue-700 border-blue-100">
                                        ...
                                        <>
                                            ...
                                            <div className="space-y-1 mt-2">
                                                {item.data.medications.slice(0, 2).map((med, i) => (
                                                    <div key={i} className="text-[11px] text-slate-500 dark:text-slate-400 dark:text-slate-500 flex items-center gap-1.5 pl-2 border-l border-slate-200 dark:border-slate-700">
                                                        <span className="font-bold text-slate-700 dark:text-slate-200">{med.name}</span>
                    ...
                    {timelineItems.length === 0 && !patient.lastVisit && (
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <p className="text-sm text-slate-400 dark:text-slate-500 font-medium">Nenhum evento registrado no histórico clínico.</p>
                        </div>
                    )}
                </div>
            </CardContent>
