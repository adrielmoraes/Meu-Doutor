      ...
      <CardContent className="p-4 flex flex-col md:flex-row gap-6 items-start md:items-center">
        ...
        {allergies.length > 0 ? (
          ...
          <div className="flex items-center gap-3 opacity-60">
            ...
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-tight">Sem alergias registradas</span>
          </div>
        )}
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className="text-[10px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">
          ...
          <div className="flex flex-wrap gap-2">
            {preventiveAlerts.map((alert, idx) => (
              <TooltipProvider key={idx}>
                <Tooltip>
                  <TooltipTrigger>
                    <Badge variant="outline" className="bg-amber-50 dark:bg-amber-950/30 text-amber-900 border-amber-200 dark:border-amber-800 hover:bg-amber-100 shadow-sm">
            ...
            {activeConditions.map((cond, idx) => (
              <Badge key={idx} variant="secondary" className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700">
                {cond}
              </Badge>
            ))}
            {preventiveAlerts.length === 0 && activeConditions.length === 0 && (
              <span className="text-xs text-slate-400 dark:text-slate-500 italic">Nenhum alerta clínico ativo.</span>
            )}
          </div>
        </div>
